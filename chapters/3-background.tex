...
From the example above, Anne has the local state $(\mathcal{M},\{w_1,w_2\})$ and Bill has the local state $(\mathcal{M},\{w_2,w_3\})$.
...
From the example above, in $(M,w_2)$, $w_2$ is the designated world. This designated world says nothing about the knowledge of Anne. She still does not know if $q$ is true or not. $(M,w_2) \models \neg K_aq \vee \neg K_a \neg q$.
...
then the product update of $s$ with $a$ is defined as $s \otimes a = ((W',(\sim_i')_{i \in \mathcal{A}}, W_d'))$ where :
 ...
 \begin{itemize}
   ...
   \item propositions become true if the proposition occurred as a positive literall in the effect of the event or the proposition was satisfied in the world before and did not appear negative in the effect \\
   ...
   emerge from designated predecessor worlds and designated events \\
...
\begin{enumerate}
  ...
  \item Sensing actions \\
    ...
    %
    \begin{tikzpicture}
      \draw (-2,0) node[world, label=below:{$e_1: \langle \top, qR \rangle $}] (e1) {};
      \draw (0,0) node [desig] {}; % designation
      \draw (0,0) node[world, label=below:{$e_2: \langle \top, qR \rangle $}] (e2) {};
      \draw (e1) -- node[above] {Max} (e2);
    \end{tikzpicture}
    %
    ~=~
    %
...
\begin{enumerate}
  ...
  \item Uniformity \\
    ...
    In the planning task between Lea and Max, a policy that perscribes him to pick up the queen right after Lea put down the cards is not a valid policy.
...
In the lever example, a policy could be that, starting in the position with the lever being in the middle, Bill pulls the lever to the right and then to the right again. This is a policiy that satisfies all properties and where the last state satisfies the goal formula. The execution of the policy is finite and successful. \\
...
The cost of a apolicy can be defined as its worst-case execution length, the number of actions in its longest possible execution. An optimal policy in one with minimal costs. Due to partial observability and different knowledge, the agents might assign different costs to the same policy and therefore the costs are subjective. \\
Let $\pi$ be a strong policy for a planning task $\Pi$. The perspecitve-sensitive cost (or simply cost) of $\pi$ from a state $s\in Dom(\pi)$, denoted $\kappa_{\pi}(s)$ is defined as:
$$\kappa_{\pi}(s) =
\begin{cases}
